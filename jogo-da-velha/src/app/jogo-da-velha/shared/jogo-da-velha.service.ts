...
export class JogoDaVelhaService {
  ...
  private inicializarTabuleiro(): void {
    ...
    for (let i = 0; i < this.TAM_TAB; i++) {
      this.tabuleiro[i] = [this.EMPTY, this.EMPTY, this.EMPTY]
    }
  }
  ...
  public jogar(posX: number, posY: number): void {
    if (this.tabuleiro[posX][posY] !== this.EMPTY || this.vitoria) {
      return;
    }
  ...
  private fimJogo(linha: number, coluna: number, tabuleiro: any, jogador: number): false|number[][] {
    let fim: false|number[][] = false;

    if (tabuleiro[0][coluna] === jogador && tabuleiro[1][coluna] === jogador && tabuleiro[2][coluna] === jogador) {
      fim = [[0, coluna], [1, coluna], [2, coluna]];
    }

    if (tabuleiro[linha][0] === jogador && tabuleiro[linha][0] === jogador && tabuleiro[linha][2] === jogador) {
      fim = [[linha, 0], [linha, 1], [linha, 3]];
    }
    ...
    if (jogada.length <= 0) {
      ...
      for (let i = 0; i < this.TAM_TAB; i++) {
        for (let j = 0; j < this.TAM_TAB; j++) {
          if (this.tabuleiro[i][j] === this.EMPTY) {
            jogadas.push([i,j]);
          }
        }
      }
  ...
  public obterJogada(jogador: number): number[] {
    ...
    for (let lin = 0; lin < this.TAM_TAB; lin++) {
      for (let col = 0; col < this.TAM_TAB; lin++) {
